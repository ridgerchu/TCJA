...
class SEWResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, cnf: str = None, single_step_neuron: callable = neuron.MultiStepLIFNode(), **kwargs):
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
class MultiStepSEWResNet_dynamic(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, T:int=None, cnf: str=None, multi_step_neuron: callable = None, **kwargs):
        super().__init__()
        ...
        self.conv1 = nn.Conv2d(2, self.inplanes, kernel_size=[7, 7], stride=[2, 2], padding=[3, 3],
                               bias=False)
        self.att = layer.TCJA(4, 8, self.inplanes)
        self.bn1 = norm_layer(self.inplanes)
        self.sn1 = multi_step_neuron(**kwargs)
    ...
    def _make_att_layer(self, block, planes, blocks, stride=1, dilate=False, cnf: str = None, multi_step_neuron: callable = None, **kwargs):
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
        ...
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, cnf=cnf, multi_step_neuron=multi_step_neuron, **kwargs))
            layers.append(layer.TCJA(4, self.T, self.inplanes))
    ...
    def _forward_impl(self, x: torch.Tensor):
        ...
        x = functional.seq_to_ann_forward(x, self.bn1)

        x_seq = functional.seq_to_ann_forward(x, self.maxpool)
        x_seq = self.att(x_seq)
    ...
    def forward(self, x):
        ...
        out_spikes = self._forward_impl(x)  # shape = [T, N, 110]
...
def _multi_step_sew_resnet(arch, block, layers, pretrained, progress, T, cnf, multi_step_neuron, **kwargs):
    model = MultiStepSEWResNet_dynamic(block, layers, T=T, cnf=cnf, multi_step_neuron=multi_step_neuron, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
def sew_resnet18(pretrained=False, progress=True, cnf: str = None, single_step_neuron: callable=neuron.MultiStepLIFNode(), **kwargs):
    """
...
def multi_step_sew_wide_resnet101_2(pretrained=False, progress=True, T: int = None, cnf: str = None, multi_step_neuron: callable=None, **kwargs):
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _multi_step_sew_resnet('wide_resnet101_2', MultiStepBottleneck, [3, 4, 23, 3], pretrained, progress, T, cnf, multi_step_neuron, **kwargs)